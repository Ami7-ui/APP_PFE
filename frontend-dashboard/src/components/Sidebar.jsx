  ...
  return (
    <aside style={{
      ...
      <div className="glass-panel" style={{
        padding: '16px', display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20,
      ...
      <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.2em', padding: '12px 10px 4px' }}>
        Menu Principal
      </div>
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {visibleNav.map(n => (
          ...
          >
            {({ isActive }) => (
              <>
                <n.Icon size={20} color={isActive ? 'var(--accent-cyan)' : 'var(--text-muted)'} strokeWidth={isActive ? 2.5 : 2} style={{ transition: 'color 0.2s', filter: isActive ? 'drop-shadow(0 0 8px var(--glow-cyan))' : 'none' }} />
                {n.label}
              </>
            )}
          </NavLink>
